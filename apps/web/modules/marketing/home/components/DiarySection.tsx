...
const WEATHER_ICON_MAP: Record<string, string> = {
	晴天: "https://www.huohuo90.com:443/weather/sun.svg",
	晴: "https://www.huohuo90.com:443/weather/sun.svg",
	多云: "https://www.huohuo90.com:443/weather/cloud.svg",
	阴天: "https://www.huohuo90.com:443/weather/cloud.svg",
	下雨: "https://www.huohuo90.com:443/weather/rain.svg",
	雨: "https://www.huohuo90.com:443/weather/rain.svg",
	雪: "https://www.huohuo90.com:443/weather/snow.svg",
	下雪: "https://www.huohuo90.com:443/weather/snow.svg",
};

function getWeatherIcon(weather: string): string {
	if (weather.startsWith("http")) return weather;
	return WEATHER_ICON_MAP[weather] || "https://www.huohuo90.com:443/weather/sun.svg";
}
...
function MiniCalendar() {
	...
	const prevDays: { day: number; type: "prev" | "current" | "next" }[] = [];
	for (let i = firstDay - 1; i >= 0; i--) {
		prevDays.push({ day: prevMonthDays - i, type: "prev" });
	}

	const currentDays: { day: number; type: "prev" | "current" | "next" }[] = [];
	for (let i = 1; i <= daysInMonth; i++) {
		currentDays.push({ day: i, type: "current" });
	}

	const totalCells = Math.ceil((prevDays.length + currentDays.length) / 7) * 7;
	const nextDays: { day: number; type: "prev" | "current" | "next" }[] = [];
	const remaining = totalCells - prevDays.length - currentDays.length;
	for (let i = 1; i <= remaining; i++) {
		nextDays.push({ day: i, type: "next" });
	}

	const allDays = [...prevDays, ...currentDays, ...nextDays];
	const weeks: { day: number; type: "prev" | "current" | "next" }[][] = [];
		...
		<div className="rounded-2xl border bg-card p-4">
			<div className="mb-3 flex items-center justify-between">
				<div className="flex items-center gap-2">
					<button type="button" onClick={prevMonth} className="p-1 text-foreground/50 hover:text-foreground">
						<ChevronLeftIcon className="size-4" />
					</button>
					<span className="min-w-[140px] text-center text-sm font-semibold">
						{year}年{month + 1}月{today.getDate()}日
					</span>
					<button type="button" onClick={nextMonth} className="p-1 text-foreground/50 hover:text-foreground">
						<ChevronRightIcon className="size-4" />
					</button>
					<button
						type="button"
						onClick={goToToday}
						className="ml-1 rounded bg-primary/10 px-2 py-0.5 text-xs text-primary"
					>
						今天
					</button>
				</div>
			...
			<table className="w-full text-center text-xs">
				<thead>
					<tr>
						{WEEKDAYS.map((d) => (
							<th key={d} className="py-1.5 font-medium text-foreground/50">
								{d}
							</th>
						))}
					</tr>
...
export function DiarySection({ latestEntry }: { latestEntry: LatestDiaryEntry | null }) {
	return (
		<section id="diary" className="container py-16">
			{/* Title */}
			<div className="mb-8">
				<div className="flex items-center gap-3">
					<h2 className="text-2xl font-bold">随笔随记</h2>
					<p className="text-sm text-foreground/50">我的日常记录...</p>
				</div>
			</div>
			...
			<div className="flex flex-col gap-10 lg:flex-row">
				...
				<div className="flex flex-col gap-6 lg:w-72">
					...
					<div className="flex flex-col gap-2 text-foreground/50">
						<p className="text-lg">那些</p>
						<p className="text-xl font-medium text-foreground/70">浅浅的叙述</p>
					...
					<div className="relative">
						...
						{latestEntry ? (
							<div className="relative p-6 md:p-8">
								{/* Diary header */}
								<div className="mb-4">
									<h3 className="text-xl font-bold">{latestEntry.title}</h3>
									<div className="mt-2 flex items-center gap-3">
										<span className="text-sm text-foreground/50">
											{formatDate(latestEntry.date)}
										</span>
										<img
											src={getWeatherIcon(latestEntry.weather)}
											alt="weather"
											className="size-5"
										/>
									</div>
								...
								{/* Diary content */}
								<div className="max-h-[500px] overflow-y-auto pr-2 whitespace-pre-line text-sm leading-relaxed text-foreground/70">
									{latestEntry.content}
								</div>
								...
								{latestEntry.image && (
									<div className="mt-6 flex gap-4">
										<div className="h-[200px] w-[160px] overflow-hidden rounded-lg">
											<img
								...
								<div className="mt-6 flex justify-end">
									<svg
										...
										className="size-10 text-foreground/10"
										fill="currentColor"
									>
										<title>逸刻时光</title>
							...
							<div className="relative flex items-center justify-center p-16">
								<p className="text-foreground/40">暂无随记</p>
							</div>
						)}
					</div>
				</div>
