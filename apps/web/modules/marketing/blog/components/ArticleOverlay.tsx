...
export function ArticleOverlay({
	...
	return (
		<div className="fixed inset-0 z-50 flex items-start justify-center">
			...
			{/* Content */}
			<div className="relative z-10 mt-8 mb-8 flex w-full max-w-[900px] max-h-[calc(100vh-64px)] overflow-y-auto rounded-2xl bg-background shadow-2xl">
				{/* Close button */}
				<button
					type="button"
					onClick={onClose}
					className="absolute top-4 right-4 z-20 rounded-full bg-foreground/5 p-2 text-foreground/50 transition-colors hover:bg-foreground/10 hover:text-foreground"
				>
				...
				<div className="flex-1 min-w-0 p-8 md:p-10">
					...
					<div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
						<span className="text-sm text-foreground/50">
							{article.category}{" "}
							<span className="text-foreground/30">/</span> {article.tag}
						</span>
						<span className="text-sm text-foreground/40">{article.date}</span>
					</div>

					{/* Stats row */}
					<div className="mt-3 flex items-center gap-4">
						<span className="flex items-center gap-1.5 text-sm text-foreground/40">
					...
					<div className="mt-6 rounded-lg border-l-4 border-primary/30 bg-primary/5 px-4 py-3">
						<p className="text-sm text-foreground/60">{article.summary}</p>
					</div>

					{/* Cover image */}
					{article.image && (
						<div className="relative mt-6 h-[400px] overflow-hidden rounded-xl">
							<Image
								src={article.image}
								alt={article.title}
								fill
					...
					<div className="mt-8">
						<div
							className="prose prose-neutral dark:prose-invert max-w-[800px] whitespace-pre-wrap text-[15px] leading-loose text-foreground/70"
				...
				{/* Side toolbar */}
				<div className="sticky top-8 flex h-fit shrink-0 flex-col items-center gap-3 p-4 pt-20">
					{/* Like */}
					<button
						type="button"
						...
						className={cn(
							...
							liked
								...
								: "bg-card text-foreground/40 hover:bg-foreground/5 hover:text-foreground/60",
						)}
					>
						<HeartIcon
					...
					<button
						type="button"
						className="flex flex-col items-center gap-1 rounded-xl border bg-card p-3 text-foreground/40 transition-colors hover:bg-foreground/5 hover:text-foreground/60"
					>
					...
					<div className="relative">
						...
						{/* Share popover */}
						{showShare && (
							<div className="absolute right-full top-0 mr-3 w-[260px] rounded-xl border bg-card p-5 shadow-lg">
								<div className="absolute top-3 -right-1.5 size-3 rotate-45 border-r border-t bg-card" />
								<p className="mb-3 font-semibold">分享</p>
								...
								<div className="mt-4 flex gap-2">
									<button
										...
										className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-sm transition-colors hover:bg-foreground/5"
									>
